...
public class NhanKhauService {
    ...
    public boolean themNhanKhauTrongTamTru(NhanKhauBean nhanKhauBean) {
        try {
            NhanKhauModel nhanKhau = nhanKhauBean.getNhanKhauModel();
            ChungMinhThuModel chungMinhThu = nhanKhauBean.getChungMinhThuModel();
            Connection connection = MysqlConnection.getMysqlConnection();

            String query = "INSERT INTO nhan_khau (hoTen, noiThuongTru, namSinh, idNguoiTao, ngayTao, diaChiMoi)"
                    + " values (?, ?, ?, ?, ?, ?)";
            PreparedStatement preparedStatement = connection.prepareStatement(query, Statement.RETURN_GENERATED_KEYS);
            preparedStatement.setString(1, nhanKhau.getHoTen());
            preparedStatement.setString(2, nhanKhau.getNoiThuongTru());
            ...
            preparedStatement.setString(6, nhanKhau.getDiaChiMoi());
            System.out.println(query);
            ...
            if (rs.next()) {
                genID = rs.getInt(1);
                String sql = "INSERT INTO chung_minh_thu(idNhanKhau, soCMT) values (?, ?)";
            ...
            String query2 = "UPDATE nhan_khau SET diaChiMoi = ? WHERE ID = ?";
            PreparedStatement st = connection.prepareStatement(query2);
            st.setString(1, nhanKhau.getDiaChiMoi());
            st.setInt(2, genID);
            st.executeUpdate();
            nhanKhau.setDiaChiMoi(nhanKhau.getDiaChiMoi());
